...
function bp_group_documents_display_content() {
	...
	<?php if( $template->document_list && count($template->document_list >= 1) ) { ?>
		...
		foreach( $template->document_list as $document_params ) {
			...
			<span class="group-documents-meta"><?php printf( __( 'Uploaded by %s on %s', 'bp-group-documents'),bp_core_get_userlink($document->user_id),date_i18n( get_option('date_format'), $document->created_ts )); ?></span>
